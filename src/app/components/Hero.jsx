import React from "react";
import Image from "next/image";
import BannerMan from "../../img/hero-person-img.png";
import certificate from "../../img/icons/certificate.svg";

const Hero = () => {
  return (
    <section>
      <div className="bg-[#FDF8F7]">
        <div className="container mx-auto">
          <div
            className=" grid grid-cols-1 md:grid-cols-2 pt-[155px]    items-center 
          
           justify-items-center
          "
          >
            {/* hero content */}
            <div className="pt-10 pb-[45px] p-4 md:p-0">
              <span className="text-2xl text-[#FD6E0A] font-bold">Hi I m</span>
              <h1 className="text-[70px] text-[#15295F] font-bold leading-[1.3]">
                Mary Hardy
              </h1>
              <h3 className="  leading-tight mt-4  text-[#15295F] font-semibold">
                Digital Marketing Expert
              </h3>
              <p className="my-5 text-left pr-5  text-[#8F93A8]">
                Shot what able cold new the see hold. Friendly as an betrayed
                formerly he. Morning because as to society behaved moments
              </p>

              <div className=" cta__buttons space-x-2 mt-8">
                <button
                  className=" 
                  
                  hover:bg-transparent
                  hover:border-primary
                  hover:text-[#FD6E0A]

                   transition-all
                     duration-300
                  border-2
                    border-transparent
                  
                  px-8 py-3 rounded-full bg-[#FD6E0A] text-white "
                >
                  Downlod Cv
                </button>
                <button
                  className=" px-8 py-3 
                  
                
                  rounded-full  border-2 border-[#FD6E0A] text-[#FD6E0A] "
                >
                  Contact
                </button>
              </div>
            </div>

            {/* hero imge */}
            <div className="   order-first md:order-last">
              <Image src={BannerMan} alt="man" height={600} width={600} />
            </div>
          </div>
        </div>
      </div>

      {/* hero pannel  for md to large devices*/}

      <div className="hero__panel hidden md:block lg:-mt-[6rem] mt-[3rem] ">
        <div className="    flex justify-center items-center">
          {/* pannle wrapper */}

          <div className=" px-2">
            <div
              className={` customShadow   px-[5rem] py-[2rem]  bg-[#fff] rounded-full grid grid-cols-3  md:gap-x-[12rem] 
           
          lg:gap-x-[8rem] `}
            >
              {/* items 1 */}

              <div className=" flex ">
                {/* main content */}

                <div
                  className=" 
                lg:space-y-0 space-y-3   flex items-center justify-center lg:space-x-3  lg:flex-row flex-col"
                >
                  {/* logo */}
                  <div className=" p-3 bg-primary  rounded-full shadow-md flex justify-center items-center  h-[5rem] w-[5rem]">
                    <svg
                      width={40}
                      height={40}
                      viewBox="0 0 40 40"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clipPath="url(#clip0_37_269)">
                        <path
                          d="M24.5241 16.2844L18.5141 21.961L15.6297 18.9936C15.31 18.6644 14.7834 18.6569 14.4546 18.9766C14.1253 19.2967 14.1179 19.8229 14.4376 20.1521L17.897 23.7034C18.2184 24.0267 18.7378 24.0378 19.0725 23.7281L25.683 17.4931C25.8435 17.3417 25.9373 17.1326 25.9435 16.9122C25.9497 16.6918 25.868 16.4778 25.7162 16.3176C25.3945 15.9832 24.8641 15.9685 24.5241 16.2844Z"
                          fill="white"
                        />
                        <path
                          d="M20.0645 7.54828C13.1958 7.54828 7.61072 13.1334 7.61072 20.002C7.61072 26.8711 13.1958 32.4561 20.0645 32.4561C26.9335 32.4561 32.5186 26.8711 32.5186 20.002C32.5186 13.1334 26.9335 7.54828 20.0645 7.54828ZM20.0645 30.7889C14.1128 30.7889 9.27791 25.9456 9.27791 20.002C9.27791 14.0585 14.1128 9.21547 20.0645 9.21547C26.0221 9.21547 30.8514 14.0448 30.8514 20.002C30.8514 25.9593 26.0221 30.7889 20.0645 30.7889Z"
                          fill="white"
                        />
                        <path
                          d="M37.2033 15.4173L37.4784 10.474C37.4976 10.1422 37.3172 9.83057 37.02 9.68208L32.6185 7.45645L30.3929 3.05501C30.2415 2.76032 29.9318 2.5809 29.601 2.59653L24.6492 2.86355L20.5229 0.137433C20.2448 -0.0458924 19.8844 -0.0458924 19.606 0.137433L15.4797 2.86355L10.5367 2.58839C10.2046 2.56886 9.89329 2.74925 9.74481 3.04687L7.51885 7.44831L3.11773 9.67394C2.82272 9.82503 2.64363 10.1347 2.65926 10.4659L2.93408 15.4091L0.208295 19.5354C0.0249694 19.8135 0.0249694 20.174 0.208295 20.4524L2.92594 24.5868L2.65079 29.5301C2.63158 29.8619 2.81165 30.1735 3.10927 30.322L7.51071 32.5476L9.73634 36.9491C9.88743 37.2441 10.1971 37.4232 10.5283 37.4075L15.4715 37.1324L19.5978 39.8582C19.8743 40.0474 20.2383 40.0474 20.5148 39.8582L24.6411 37.1324L29.5844 37.4075C29.9162 37.4268 30.2275 37.2464 30.3763 36.9491L32.6019 32.5476L37.0033 30.322C37.298 30.1706 37.4775 29.8612 37.4618 29.5301L37.1867 24.5868L39.9125 20.4605C40.0958 20.1824 40.0958 19.8216 39.9125 19.5436L37.2033 15.4173ZM35.6527 23.9033C35.5511 24.0525 35.5039 24.2319 35.5195 24.4119L35.7777 29.0882L31.6182 31.1888C31.4596 31.2683 31.331 31.3972 31.2515 31.5558L29.1506 35.7153L24.4744 35.4571C24.2953 35.4506 24.1185 35.4968 23.9657 35.5902L20.0644 38.1662L16.1635 35.5902C16.028 35.4987 15.8685 35.4492 15.705 35.4486H15.6633L10.9867 35.7071L8.88614 31.5473C8.80669 31.3887 8.67807 31.2601 8.51917 31.1807L4.35119 29.0882L4.60974 24.4119C4.61625 24.2329 4.56969 24.056 4.47623 23.9033L1.90056 20.002L4.47623 16.1008C4.57815 15.9519 4.62537 15.7722 4.60974 15.5925L4.35119 10.9159L8.51103 8.81527C8.6696 8.73582 8.79822 8.6072 8.87768 8.44862L10.9783 4.28879L15.6549 4.54734C15.8336 4.55353 16.0104 4.50729 16.1635 4.41383L20.0644 1.83816L23.9657 4.41383C24.1149 4.51575 24.2946 4.56297 24.4744 4.54734L29.1506 4.28879L31.2515 8.44862C31.331 8.6072 31.4596 8.73582 31.6182 8.81527L35.7777 10.9159L35.5195 15.5925C35.513 15.7712 35.5595 15.948 35.6527 16.1008L38.2287 20.002L35.6527 23.9033Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_37_269">
                          <rect width={40} height={40} fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                  {/* text */}

                  <div className=" text-center lg:text-left">
                    <p className=" font-bold text-[1.5rem]   text-secend ">
                      {" "}
                      8 years job
                    </p>
                    <p className=" text-primary "> Experience</p>
                  </div>
                </div>

                {/* border */}

                {/* <div className=" w-[1px] h-[98%] bg-[#f2f2f2]"></div> */}
              </div>

              {/* items 2*/}

              <div className=" flex ">
                <div className=" lg:space-y-0 space-y-3 flex items-center justify-center lg:space-x-3 lg:flex-row flex-col">
                  {/* logo */}
                  <div className=" p-5 bg-primary  rounded-full shadow-md flex justify-center items-center  h-[5rem] w-[5rem]">
                    <svg
                      width={40}
                      height={40}
                      viewBox="0 0 40 40"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clipPath="url(#clip0_37_269)">
                        <path
                          d="M24.5241 16.2844L18.5141 21.961L15.6297 18.9936C15.31 18.6644 14.7834 18.6569 14.4546 18.9766C14.1253 19.2967 14.1179 19.8229 14.4376 20.1521L17.897 23.7034C18.2184 24.0267 18.7378 24.0378 19.0725 23.7281L25.683 17.4931C25.8435 17.3417 25.9373 17.1326 25.9435 16.9122C25.9497 16.6918 25.868 16.4778 25.7162 16.3176C25.3945 15.9832 24.8641 15.9685 24.5241 16.2844Z"
                          fill="white"
                        />
                        <path
                          d="M20.0645 7.54828C13.1958 7.54828 7.61072 13.1334 7.61072 20.002C7.61072 26.8711 13.1958 32.4561 20.0645 32.4561C26.9335 32.4561 32.5186 26.8711 32.5186 20.002C32.5186 13.1334 26.9335 7.54828 20.0645 7.54828ZM20.0645 30.7889C14.1128 30.7889 9.27791 25.9456 9.27791 20.002C9.27791 14.0585 14.1128 9.21547 20.0645 9.21547C26.0221 9.21547 30.8514 14.0448 30.8514 20.002C30.8514 25.9593 26.0221 30.7889 20.0645 30.7889Z"
                          fill="white"
                        />
                        <path
                          d="M37.2033 15.4173L37.4784 10.474C37.4976 10.1422 37.3172 9.83057 37.02 9.68208L32.6185 7.45645L30.3929 3.05501C30.2415 2.76032 29.9318 2.5809 29.601 2.59653L24.6492 2.86355L20.5229 0.137433C20.2448 -0.0458924 19.8844 -0.0458924 19.606 0.137433L15.4797 2.86355L10.5367 2.58839C10.2046 2.56886 9.89329 2.74925 9.74481 3.04687L7.51885 7.44831L3.11773 9.67394C2.82272 9.82503 2.64363 10.1347 2.65926 10.4659L2.93408 15.4091L0.208295 19.5354C0.0249694 19.8135 0.0249694 20.174 0.208295 20.4524L2.92594 24.5868L2.65079 29.5301C2.63158 29.8619 2.81165 30.1735 3.10927 30.322L7.51071 32.5476L9.73634 36.9491C9.88743 37.2441 10.1971 37.4232 10.5283 37.4075L15.4715 37.1324L19.5978 39.8582C19.8743 40.0474 20.2383 40.0474 20.5148 39.8582L24.6411 37.1324L29.5844 37.4075C29.9162 37.4268 30.2275 37.2464 30.3763 36.9491L32.6019 32.5476L37.0033 30.322C37.298 30.1706 37.4775 29.8612 37.4618 29.5301L37.1867 24.5868L39.9125 20.4605C40.0958 20.1824 40.0958 19.8216 39.9125 19.5436L37.2033 15.4173ZM35.6527 23.9033C35.5511 24.0525 35.5039 24.2319 35.5195 24.4119L35.7777 29.0882L31.6182 31.1888C31.4596 31.2683 31.331 31.3972 31.2515 31.5558L29.1506 35.7153L24.4744 35.4571C24.2953 35.4506 24.1185 35.4968 23.9657 35.5902L20.0644 38.1662L16.1635 35.5902C16.028 35.4987 15.8685 35.4492 15.705 35.4486H15.6633L10.9867 35.7071L8.88614 31.5473C8.80669 31.3887 8.67807 31.2601 8.51917 31.1807L4.35119 29.0882L4.60974 24.4119C4.61625 24.2329 4.56969 24.056 4.47623 23.9033L1.90056 20.002L4.47623 16.1008C4.57815 15.9519 4.62537 15.7722 4.60974 15.5925L4.35119 10.9159L8.51103 8.81527C8.6696 8.73582 8.79822 8.6072 8.87768 8.44862L10.9783 4.28879L15.6549 4.54734C15.8336 4.55353 16.0104 4.50729 16.1635 4.41383L20.0644 1.83816L23.9657 4.41383C24.1149 4.51575 24.2946 4.56297 24.4744 4.54734L29.1506 4.28879L31.2515 8.44862C31.331 8.6072 31.4596 8.73582 31.6182 8.81527L35.7777 10.9159L35.5195 15.5925C35.513 15.7712 35.5595 15.948 35.6527 16.1008L38.2287 20.002L35.6527 23.9033Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_37_269">
                          <rect width={40} height={40} fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                  {/* text */}

                  <div className=" text-center lg:text-left">
                    <p className=" font-bold text-[1.5rem]   text-secend ">
                      {" "}
                      8 years job
                    </p>
                    <p className=" text-primary "> Experience</p>
                  </div>
                </div>

                {/* border */}

                {/* <div className=" w-[1px] h-[98%] bg-[#f2f2f2]"></div> */}
              </div>
              {/* items 3 */}

              <div>
                <div className=" lg:space-y-0 space-y-3 flex items-center justify-center lg:space-x-3 lg:flex-row flex-col">
                  {/* logo */}
                  <div className=" p-5 bg-primary  rounded-full shadow-md flex justify-center items-center  h-[5rem] w-[5rem]">
                    <svg
                      width={40}
                      height={40}
                      viewBox="0 0 40 40"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clipPath="url(#clip0_37_269)">
                        <path
                          d="M24.5241 16.2844L18.5141 21.961L15.6297 18.9936C15.31 18.6644 14.7834 18.6569 14.4546 18.9766C14.1253 19.2967 14.1179 19.8229 14.4376 20.1521L17.897 23.7034C18.2184 24.0267 18.7378 24.0378 19.0725 23.7281L25.683 17.4931C25.8435 17.3417 25.9373 17.1326 25.9435 16.9122C25.9497 16.6918 25.868 16.4778 25.7162 16.3176C25.3945 15.9832 24.8641 15.9685 24.5241 16.2844Z"
                          fill="white"
                        />
                        <path
                          d="M20.0645 7.54828C13.1958 7.54828 7.61072 13.1334 7.61072 20.002C7.61072 26.8711 13.1958 32.4561 20.0645 32.4561C26.9335 32.4561 32.5186 26.8711 32.5186 20.002C32.5186 13.1334 26.9335 7.54828 20.0645 7.54828ZM20.0645 30.7889C14.1128 30.7889 9.27791 25.9456 9.27791 20.002C9.27791 14.0585 14.1128 9.21547 20.0645 9.21547C26.0221 9.21547 30.8514 14.0448 30.8514 20.002C30.8514 25.9593 26.0221 30.7889 20.0645 30.7889Z"
                          fill="white"
                        />
                        <path
                          d="M37.2033 15.4173L37.4784 10.474C37.4976 10.1422 37.3172 9.83057 37.02 9.68208L32.6185 7.45645L30.3929 3.05501C30.2415 2.76032 29.9318 2.5809 29.601 2.59653L24.6492 2.86355L20.5229 0.137433C20.2448 -0.0458924 19.8844 -0.0458924 19.606 0.137433L15.4797 2.86355L10.5367 2.58839C10.2046 2.56886 9.89329 2.74925 9.74481 3.04687L7.51885 7.44831L3.11773 9.67394C2.82272 9.82503 2.64363 10.1347 2.65926 10.4659L2.93408 15.4091L0.208295 19.5354C0.0249694 19.8135 0.0249694 20.174 0.208295 20.4524L2.92594 24.5868L2.65079 29.5301C2.63158 29.8619 2.81165 30.1735 3.10927 30.322L7.51071 32.5476L9.73634 36.9491C9.88743 37.2441 10.1971 37.4232 10.5283 37.4075L15.4715 37.1324L19.5978 39.8582C19.8743 40.0474 20.2383 40.0474 20.5148 39.8582L24.6411 37.1324L29.5844 37.4075C29.9162 37.4268 30.2275 37.2464 30.3763 36.9491L32.6019 32.5476L37.0033 30.322C37.298 30.1706 37.4775 29.8612 37.4618 29.5301L37.1867 24.5868L39.9125 20.4605C40.0958 20.1824 40.0958 19.8216 39.9125 19.5436L37.2033 15.4173ZM35.6527 23.9033C35.5511 24.0525 35.5039 24.2319 35.5195 24.4119L35.7777 29.0882L31.6182 31.1888C31.4596 31.2683 31.331 31.3972 31.2515 31.5558L29.1506 35.7153L24.4744 35.4571C24.2953 35.4506 24.1185 35.4968 23.9657 35.5902L20.0644 38.1662L16.1635 35.5902C16.028 35.4987 15.8685 35.4492 15.705 35.4486H15.6633L10.9867 35.7071L8.88614 31.5473C8.80669 31.3887 8.67807 31.2601 8.51917 31.1807L4.35119 29.0882L4.60974 24.4119C4.61625 24.2329 4.56969 24.056 4.47623 23.9033L1.90056 20.002L4.47623 16.1008C4.57815 15.9519 4.62537 15.7722 4.60974 15.5925L4.35119 10.9159L8.51103 8.81527C8.6696 8.73582 8.79822 8.6072 8.87768 8.44862L10.9783 4.28879L15.6549 4.54734C15.8336 4.55353 16.0104 4.50729 16.1635 4.41383L20.0644 1.83816L23.9657 4.41383C24.1149 4.51575 24.2946 4.56297 24.4744 4.54734L29.1506 4.28879L31.2515 8.44862C31.331 8.6072 31.4596 8.73582 31.6182 8.81527L35.7777 10.9159L35.5195 15.5925C35.513 15.7712 35.5595 15.948 35.6527 16.1008L38.2287 20.002L35.6527 23.9033Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_37_269">
                          <rect width={40} height={40} fill="white" />
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                  {/* text */}

                  <div className=" text-center lg:text-left">
                    <p className=" font-bold text-[1.5rem]   text-secend ">
                      {" "}
                      8 years job
                    </p>
                    <p className=" text-primary "> Experience</p>
                  </div>
                </div>
                {/* border  */}
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* hero pannel  for md to sm devices*/}

      <div  className=" md:hidden">
        <div className="hero__panel sm:gap-x-5  p-5 grid grid-cols-1 sm:grid-cols-2  gap-y-7 ">
         
         
          <div className=" customShadow w-full py-4 rounded-full   flex justify-start items-center space-x-3  px-5">
            {/* logo */}
            <div className=" p-2 bg-primary  rounded-full shadow-md flex justify-center items-center  h-[4rem] w-[4rem]">
              <svg
                width={40}
                height={40}
                viewBox="0 0 40 40"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <g clipPath="url(#clip0_37_269)">
                  <path
                    d="M24.5241 16.2844L18.5141 21.961L15.6297 18.9936C15.31 18.6644 14.7834 18.6569 14.4546 18.9766C14.1253 19.2967 14.1179 19.8229 14.4376 20.1521L17.897 23.7034C18.2184 24.0267 18.7378 24.0378 19.0725 23.7281L25.683 17.4931C25.8435 17.3417 25.9373 17.1326 25.9435 16.9122C25.9497 16.6918 25.868 16.4778 25.7162 16.3176C25.3945 15.9832 24.8641 15.9685 24.5241 16.2844Z"
                    fill="white"
                  />
                  <path
                    d="M20.0645 7.54828C13.1958 7.54828 7.61072 13.1334 7.61072 20.002C7.61072 26.8711 13.1958 32.4561 20.0645 32.4561C26.9335 32.4561 32.5186 26.8711 32.5186 20.002C32.5186 13.1334 26.9335 7.54828 20.0645 7.54828ZM20.0645 30.7889C14.1128 30.7889 9.27791 25.9456 9.27791 20.002C9.27791 14.0585 14.1128 9.21547 20.0645 9.21547C26.0221 9.21547 30.8514 14.0448 30.8514 20.002C30.8514 25.9593 26.0221 30.7889 20.0645 30.7889Z"
                    fill="white"
                  />
                  <path
                    d="M37.2033 15.4173L37.4784 10.474C37.4976 10.1422 37.3172 9.83057 37.02 9.68208L32.6185 7.45645L30.3929 3.05501C30.2415 2.76032 29.9318 2.5809 29.601 2.59653L24.6492 2.86355L20.5229 0.137433C20.2448 -0.0458924 19.8844 -0.0458924 19.606 0.137433L15.4797 2.86355L10.5367 2.58839C10.2046 2.56886 9.89329 2.74925 9.74481 3.04687L7.51885 7.44831L3.11773 9.67394C2.82272 9.82503 2.64363 10.1347 2.65926 10.4659L2.93408 15.4091L0.208295 19.5354C0.0249694 19.8135 0.0249694 20.174 0.208295 20.4524L2.92594 24.5868L2.65079 29.5301C2.63158 29.8619 2.81165 30.1735 3.10927 30.322L7.51071 32.5476L9.73634 36.9491C9.88743 37.2441 10.1971 37.4232 10.5283 37.4075L15.4715 37.1324L19.5978 39.8582C19.8743 40.0474 20.2383 40.0474 20.5148 39.8582L24.6411 37.1324L29.5844 37.4075C29.9162 37.4268 30.2275 37.2464 30.3763 36.9491L32.6019 32.5476L37.0033 30.322C37.298 30.1706 37.4775 29.8612 37.4618 29.5301L37.1867 24.5868L39.9125 20.4605C40.0958 20.1824 40.0958 19.8216 39.9125 19.5436L37.2033 15.4173ZM35.6527 23.9033C35.5511 24.0525 35.5039 24.2319 35.5195 24.4119L35.7777 29.0882L31.6182 31.1888C31.4596 31.2683 31.331 31.3972 31.2515 31.5558L29.1506 35.7153L24.4744 35.4571C24.2953 35.4506 24.1185 35.4968 23.9657 35.5902L20.0644 38.1662L16.1635 35.5902C16.028 35.4987 15.8685 35.4492 15.705 35.4486H15.6633L10.9867 35.7071L8.88614 31.5473C8.80669 31.3887 8.67807 31.2601 8.51917 31.1807L4.35119 29.0882L4.60974 24.4119C4.61625 24.2329 4.56969 24.056 4.47623 23.9033L1.90056 20.002L4.47623 16.1008C4.57815 15.9519 4.62537 15.7722 4.60974 15.5925L4.35119 10.9159L8.51103 8.81527C8.6696 8.73582 8.79822 8.6072 8.87768 8.44862L10.9783 4.28879L15.6549 4.54734C15.8336 4.55353 16.0104 4.50729 16.1635 4.41383L20.0644 1.83816L23.9657 4.41383C24.1149 4.51575 24.2946 4.56297 24.4744 4.54734L29.1506 4.28879L31.2515 8.44862C31.331 8.6072 31.4596 8.73582 31.6182 8.81527L35.7777 10.9159L35.5195 15.5925C35.513 15.7712 35.5595 15.948 35.6527 16.1008L38.2287 20.002L35.6527 23.9033Z"
                    fill="white"
                  />
                </g>
                <defs>
                  <clipPath id="clip0_37_269">
                    <rect width={40} height={40} fill="white" />
                  </clipPath>
                </defs>
              </svg>
            </div>
            {/* text */}

            <div className=" ">
              <p className=" font-bold text-[1.5rem]   text-secend ">
                {" "}
                8 years job
              </p>
              <p className=" text-primary "> Experience</p>
            </div>
          </div>
          <div className=" customShadow w-full py-4 rounded-full   flex justify-start items-center space-x-3  px-5">
            {/* logo */}
            <div className=" p-2 bg-primary  rounded-full shadow-md flex justify-center items-center  h-[4rem] w-[4rem]">
              <svg
                width={40}
                height={40}
                viewBox="0 0 40 40"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <g clipPath="url(#clip0_37_269)">
                  <path
                    d="M24.5241 16.2844L18.5141 21.961L15.6297 18.9936C15.31 18.6644 14.7834 18.6569 14.4546 18.9766C14.1253 19.2967 14.1179 19.8229 14.4376 20.1521L17.897 23.7034C18.2184 24.0267 18.7378 24.0378 19.0725 23.7281L25.683 17.4931C25.8435 17.3417 25.9373 17.1326 25.9435 16.9122C25.9497 16.6918 25.868 16.4778 25.7162 16.3176C25.3945 15.9832 24.8641 15.9685 24.5241 16.2844Z"
                    fill="white"
                  />
                  <path
                    d="M20.0645 7.54828C13.1958 7.54828 7.61072 13.1334 7.61072 20.002C7.61072 26.8711 13.1958 32.4561 20.0645 32.4561C26.9335 32.4561 32.5186 26.8711 32.5186 20.002C32.5186 13.1334 26.9335 7.54828 20.0645 7.54828ZM20.0645 30.7889C14.1128 30.7889 9.27791 25.9456 9.27791 20.002C9.27791 14.0585 14.1128 9.21547 20.0645 9.21547C26.0221 9.21547 30.8514 14.0448 30.8514 20.002C30.8514 25.9593 26.0221 30.7889 20.0645 30.7889Z"
                    fill="white"
                  />
                  <path
                    d="M37.2033 15.4173L37.4784 10.474C37.4976 10.1422 37.3172 9.83057 37.02 9.68208L32.6185 7.45645L30.3929 3.05501C30.2415 2.76032 29.9318 2.5809 29.601 2.59653L24.6492 2.86355L20.5229 0.137433C20.2448 -0.0458924 19.8844 -0.0458924 19.606 0.137433L15.4797 2.86355L10.5367 2.58839C10.2046 2.56886 9.89329 2.74925 9.74481 3.04687L7.51885 7.44831L3.11773 9.67394C2.82272 9.82503 2.64363 10.1347 2.65926 10.4659L2.93408 15.4091L0.208295 19.5354C0.0249694 19.8135 0.0249694 20.174 0.208295 20.4524L2.92594 24.5868L2.65079 29.5301C2.63158 29.8619 2.81165 30.1735 3.10927 30.322L7.51071 32.5476L9.73634 36.9491C9.88743 37.2441 10.1971 37.4232 10.5283 37.4075L15.4715 37.1324L19.5978 39.8582C19.8743 40.0474 20.2383 40.0474 20.5148 39.8582L24.6411 37.1324L29.5844 37.4075C29.9162 37.4268 30.2275 37.2464 30.3763 36.9491L32.6019 32.5476L37.0033 30.322C37.298 30.1706 37.4775 29.8612 37.4618 29.5301L37.1867 24.5868L39.9125 20.4605C40.0958 20.1824 40.0958 19.8216 39.9125 19.5436L37.2033 15.4173ZM35.6527 23.9033C35.5511 24.0525 35.5039 24.2319 35.5195 24.4119L35.7777 29.0882L31.6182 31.1888C31.4596 31.2683 31.331 31.3972 31.2515 31.5558L29.1506 35.7153L24.4744 35.4571C24.2953 35.4506 24.1185 35.4968 23.9657 35.5902L20.0644 38.1662L16.1635 35.5902C16.028 35.4987 15.8685 35.4492 15.705 35.4486H15.6633L10.9867 35.7071L8.88614 31.5473C8.80669 31.3887 8.67807 31.2601 8.51917 31.1807L4.35119 29.0882L4.60974 24.4119C4.61625 24.2329 4.56969 24.056 4.47623 23.9033L1.90056 20.002L4.47623 16.1008C4.57815 15.9519 4.62537 15.7722 4.60974 15.5925L4.35119 10.9159L8.51103 8.81527C8.6696 8.73582 8.79822 8.6072 8.87768 8.44862L10.9783 4.28879L15.6549 4.54734C15.8336 4.55353 16.0104 4.50729 16.1635 4.41383L20.0644 1.83816L23.9657 4.41383C24.1149 4.51575 24.2946 4.56297 24.4744 4.54734L29.1506 4.28879L31.2515 8.44862C31.331 8.6072 31.4596 8.73582 31.6182 8.81527L35.7777 10.9159L35.5195 15.5925C35.513 15.7712 35.5595 15.948 35.6527 16.1008L38.2287 20.002L35.6527 23.9033Z"
                    fill="white"
                  />
                </g>
                <defs>
                  <clipPath id="clip0_37_269">
                    <rect width={40} height={40} fill="white" />
                  </clipPath>
                </defs>
              </svg>
            </div>
            {/* text */}

            <div className=" ">
              <p className=" font-bold text-[1.5rem]   text-secend ">
                {" "}
                8 years job
              </p>
              <p className=" text-primary "> Experience</p>
            </div>
          </div>

 

          <div className="   sm:col-span-2 sm:w-[50%] sm:mx-auto  customShadow w-full py-4 rounded-full   flex justify-start items-center space-x-3  px-5">
            {/* logo */}
            <div className=" p-2 bg-primary  rounded-full shadow-md flex justify-center items-center  h-[4rem] w-[4rem]">
              <svg
                width={40}
                height={40}
                viewBox="0 0 40 40"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <g clipPath="url(#clip0_37_269)">
                  <path
                    d="M24.5241 16.2844L18.5141 21.961L15.6297 18.9936C15.31 18.6644 14.7834 18.6569 14.4546 18.9766C14.1253 19.2967 14.1179 19.8229 14.4376 20.1521L17.897 23.7034C18.2184 24.0267 18.7378 24.0378 19.0725 23.7281L25.683 17.4931C25.8435 17.3417 25.9373 17.1326 25.9435 16.9122C25.9497 16.6918 25.868 16.4778 25.7162 16.3176C25.3945 15.9832 24.8641 15.9685 24.5241 16.2844Z"
                    fill="white"
                  />
                  <path
                    d="M20.0645 7.54828C13.1958 7.54828 7.61072 13.1334 7.61072 20.002C7.61072 26.8711 13.1958 32.4561 20.0645 32.4561C26.9335 32.4561 32.5186 26.8711 32.5186 20.002C32.5186 13.1334 26.9335 7.54828 20.0645 7.54828ZM20.0645 30.7889C14.1128 30.7889 9.27791 25.9456 9.27791 20.002C9.27791 14.0585 14.1128 9.21547 20.0645 9.21547C26.0221 9.21547 30.8514 14.0448 30.8514 20.002C30.8514 25.9593 26.0221 30.7889 20.0645 30.7889Z"
                    fill="white"
                  />
                  <path
                    d="M37.2033 15.4173L37.4784 10.474C37.4976 10.1422 37.3172 9.83057 37.02 9.68208L32.6185 7.45645L30.3929 3.05501C30.2415 2.76032 29.9318 2.5809 29.601 2.59653L24.6492 2.86355L20.5229 0.137433C20.2448 -0.0458924 19.8844 -0.0458924 19.606 0.137433L15.4797 2.86355L10.5367 2.58839C10.2046 2.56886 9.89329 2.74925 9.74481 3.04687L7.51885 7.44831L3.11773 9.67394C2.82272 9.82503 2.64363 10.1347 2.65926 10.4659L2.93408 15.4091L0.208295 19.5354C0.0249694 19.8135 0.0249694 20.174 0.208295 20.4524L2.92594 24.5868L2.65079 29.5301C2.63158 29.8619 2.81165 30.1735 3.10927 30.322L7.51071 32.5476L9.73634 36.9491C9.88743 37.2441 10.1971 37.4232 10.5283 37.4075L15.4715 37.1324L19.5978 39.8582C19.8743 40.0474 20.2383 40.0474 20.5148 39.8582L24.6411 37.1324L29.5844 37.4075C29.9162 37.4268 30.2275 37.2464 30.3763 36.9491L32.6019 32.5476L37.0033 30.322C37.298 30.1706 37.4775 29.8612 37.4618 29.5301L37.1867 24.5868L39.9125 20.4605C40.0958 20.1824 40.0958 19.8216 39.9125 19.5436L37.2033 15.4173ZM35.6527 23.9033C35.5511 24.0525 35.5039 24.2319 35.5195 24.4119L35.7777 29.0882L31.6182 31.1888C31.4596 31.2683 31.331 31.3972 31.2515 31.5558L29.1506 35.7153L24.4744 35.4571C24.2953 35.4506 24.1185 35.4968 23.9657 35.5902L20.0644 38.1662L16.1635 35.5902C16.028 35.4987 15.8685 35.4492 15.705 35.4486H15.6633L10.9867 35.7071L8.88614 31.5473C8.80669 31.3887 8.67807 31.2601 8.51917 31.1807L4.35119 29.0882L4.60974 24.4119C4.61625 24.2329 4.56969 24.056 4.47623 23.9033L1.90056 20.002L4.47623 16.1008C4.57815 15.9519 4.62537 15.7722 4.60974 15.5925L4.35119 10.9159L8.51103 8.81527C8.6696 8.73582 8.79822 8.6072 8.87768 8.44862L10.9783 4.28879L15.6549 4.54734C15.8336 4.55353 16.0104 4.50729 16.1635 4.41383L20.0644 1.83816L23.9657 4.41383C24.1149 4.51575 24.2946 4.56297 24.4744 4.54734L29.1506 4.28879L31.2515 8.44862C31.331 8.6072 31.4596 8.73582 31.6182 8.81527L35.7777 10.9159L35.5195 15.5925C35.513 15.7712 35.5595 15.948 35.6527 16.1008L38.2287 20.002L35.6527 23.9033Z"
                    fill="white"
                  />
                </g>
                <defs>
                  <clipPath id="clip0_37_269">
                    <rect width={40} height={40} fill="white" />
                  </clipPath>
                </defs>
              </svg>
            </div>
            {/* text */}

            <div className=" ">
              <p className=" font-bold text-[1.5rem]   text-secend ">
                {" "}
                8 years job
              </p>
              <p className=" text-primary "> Experience</p>
            </div>
          </div>
    


        </div>
      </div>
      {/* ======================================== */}
    </section>
  );
};

export default Hero;
